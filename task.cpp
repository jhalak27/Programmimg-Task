...
int main(){
	...
	for(int i = 0; i < N; i++){
		...
		if(c=='S'){
			// for fission
			double l, w, r; //l->length, w->width r->radius of the virus to make fission (double as they are in real number)
			cin>>l>>w>>r;
			double nr = r / pow(2, 0.5); //nr -> new raduis after fission
			double dis = pow(pow(l - 2 * nr, 2) + pow(w - 2 * nr, 2), 0.5); //max distance between centres of virus
			...
			// for fusion
			double r1, r2, d; //r1->radius of the first virus, r2->radius of the second virus d->distance between virus
			cin>>r1>>r2>>d;
			double ratio; //to store Compaction ratio 
			double area1, area2, intersection; //area1->area of first virus, area2->area of second virus, intersection->intersection area of both virus

			area1 = M_PI * r1 * r1; //area of first virus (PI*r1*r1)
			area2 = M_PI * r2 * r2; //area of first virus (PI*r2*r2)

			double angle1 = acos((d*d + r1*r1 - r2*r2) / (2 * d * r1)); //angle between lines from centre of first virus to centre of second radius and point of intersection 
			double angle2 = acos((d*d + r2*r2 - r1*r1) / (2 * d *r2)); //angle between lines from centre of second virus to centre of first radius and point of intersection 
			intersection = r2*r2*angle2 + r1*r1*angle1 - (0.5 * (r1*r1*sin(2 * angle1) + r2*r2*sin(2 * angle2))); //interestion area between virus
			
			double surfTotal = area1 + area2 - intersection; //total surface area of merged virus
